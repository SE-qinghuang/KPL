...
openai.api_key = "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"
...
def get_aspect_l(task, answer, answers, aspects):
    ...
    # get_kg_tasks(general_DIR + 'task_data.csv', task)
    kg_tasks, kg_questions, kg_answers, kg_aspects = get_kg_questions(general_DIR + 'filter_tasks.csv', answer)
    ...
    aspect_prompt += '\nUpon obtaining the query and known information, what <aspect> of the query can be subject to questioning?\n' \
                     'Please choose one <aspect> from the following options.\n'
    # aspect_prompt += 'The five <aspect>: 1. event; 2. status; 3. type; 4. purpose; 5. condition.\n' \
...
def continue_or_stop(task, answers, answer):
    continue_or_stop_prompt = 'In Java programming, unclear and unambiguous queries often have multiple APIs that can be implemented, ' \
    ...
    continue_or_stop_prompt += 'Query: "' + task + '"'
    continue_or_stop_prompt += '\nThe known clarifying information of the query: ' + ' '.join(ans for ans in answers)
    # continue_or_stop_prompt += '\nIs it still possible to clarify the query based on known information? Only answer yes or no.\n'
    continue_or_stop_prompt += '\nBased on this query and known information, is it enough to get an accurate API? Only answer yes or no.\n'
    continue_or_stops = program_Generate(prompt=continue_or_stop_prompt, temperature=0.7, max_tokens=8, num_candidates=10, stop=".")
    a = 0
    b = 0
    for cont in continue_or_stops:
        if cont.lower() == "":
            continue
        elif cont.lower() == "yes":
            a = a + 1
...
def generate_query_expansion(query, log):
    ...
    for i in range(5):
        ...
        context_prompt = "Answer the question based on the setting below.\n\nSetting:" + \
                         ...
                         "The answer should give multiple possible answers from the <" + aspect + "> aspect, and sequence these answers in the order of their relevance to the query.\n\n" + \
